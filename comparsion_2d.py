...
from rrt.rrt_star_bid_lstm_2d import RRTStarBidirectional_LSTM
from search_space.search_space import SearchSpace
from utilities.load_dataset import LoadDataset_2D 
...
if __name__ == '__main__':
    ...
    X_dimensions = np.array([(-20, 20), (-20, 20)])

    # initialize dataloader
    LoadDataset = LoadDataset_2D()
    ...
    print('init: ({:+.10f}, {:+.10f})'.format(x_init[0], x_init[1]))
    print('goal: ({:+.10f}, {:+.10f})'.format(x_goal[0], x_goal[1]))

    # create search space
    X = SearchSpace(X_dimensions, Obstacles)
    nn_path, nn_step, nn_cost_time = nn_rrt_star_bid(X, Q, x_init, x_goal, encoded_obs, max_samples, num_attempt, r, model_path, prc, rewire_count)
    rc_path, rc_step, rc_cost_time = rrt_connect(X, Q_rt, x_init, x_goal, max_samples, r, prc)
    rs_path, rs_step, rs_cost_time = rrt_star(X, Q, x_init, x_goal, max_samples, r, prc, rewire_count)
